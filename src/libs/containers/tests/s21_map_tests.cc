...
TEST(Map, Rehash_And_Insert_In_Collision) {
  ...
  for (auto i : std_map) {
    std::cout << "i = " << i.second << "; my = s21_map[i.first]"
              << "\n";
...
TEST(Map, Modifier_Erase_3) {
  s21::map<int, std::string> s21_map_1 = {
      {1, "aboba"}, {2, "shleppa"}, {3, "amogus"}, {4, "abobus"}};
  s21::map<int, std::string> s21_map_2 = {{1, "aboba"}};

  auto it0 = s21_map_1.begin();
  ++it0;
  s21_map_1.erase(it0);
  ...
  auto it2 = s21_map_1.begin();
  ++it2;
  s21_map_1.erase(it2);

  auto it3 = s21_map_1.begin();
  auto it4 = s21_map_2.begin();

  while (it3 != s21_map_1.end()) {
...
TEST(Map, emplace) {
  s21::map<int, std::string> s21_map;
  s21_map.emplace(std::make_pair(101, "20"), std::make_pair(102, "20"),
                  std::make_pair(103, "20"));

  EXPECT_EQ(s21_map.contains(101), true);
  EXPECT_EQ(s21_map.contains(102), true);
  EXPECT_EQ(s21_map.contains(103), true);
  // EXPECT_EQ(s21_map.contains(4), true);
  // EXPECT_EQ(s21_map.contains(7), false);
  // EXPECT_EQ(s21_map.contains(-2), false);
  // EXPECT_EQ(s21_map.contains(0), false);
...
TEST(Map, min_1) {
  ...
  EXPECT_EQ(it.operator*().first, 103);
  it--;

  EXPECT_EQ(it.operator*().second, "20");

  it--;
  EXPECT_EQ(it.operator*().first, 101);
  // EXPECT_EQ(s21_map.contains(4), true);
  // EXPECT_EQ(s21_map.contains(7), false);
  // EXPECT_EQ(s21_map.contains(-2), false);
  // EXPECT_EQ(s21_map.contains(0), false);
