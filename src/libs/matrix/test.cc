...
namespace TestCase {
void genMatrix(std::vector<std::vector<double>> *matrix, const int &rows, const int &cols) {
  if (rows < 0 || cols < 0) {
  ...
  for (int i = 0; i < rows; ++i) {
    for (int k = 0; k < cols; ++k) {
      (*matrix)[i].push_back(rand() % 24);
    }
  }

}

void printMatrix(std::vector<std::vector<double>> matrix, const int &rows, const int &cols) {
  for (int i = 0; i < rows; ++i) {
    cout << endl;
    for (int k = 0; k < cols; ++k) {
      cout << matrix[i][k] << " ";
...
void deleteMatrix(std::vector<std::vector<double>> *matrix, const int &rows) {
  if (rows != 0) {
    matrix->clear();
  }
}

void genS21Matrix(S21Matrix &matrix) {
  srand(time(nullptr) + rand());
...
void fillS21Matrix(S21Matrix &matrix) {
  srand(time(nullptr) + rand());
...
TEST(Getters, Matrix) {
  S21Matrix matrix(3, 4);
  std::vector<std::vector<double>> otherMatrix = matrix.GetMatrix();
...
TEST(Operators, ReturnElement) {
  S21Matrix matrix1;
  TestCase::genS21Matrix(matrix1);
  ASSERT_TRUE(matrix1.GetMatrix()[0][1] == matrix1(0, 1));
}
